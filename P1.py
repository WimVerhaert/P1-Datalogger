#!python3
# P1 Datalogger
# Release 0.1 / Sagemcom Fluvius
# Original Author J. van der Linde
# Belgian Author W. Verhaert
# Copyright (c) 2011-2017 J. van der Linde
#
...
def print_p1_telegram():
    ...
    if p1_meter_supplier == "FLU":
        print ("Meter fabrikant: Sagemcom")     
    else:
    ...
    print ("96. 1. 4 - Meternummer Elektriciteit: %s" % p1_equipment_id ) 
    print (" 1. 8. 1 - Meterstand Elektriciteit levering (T1/Normaaltarief): %0.3f %s" % (p1_meterreading_in_1,p1_unitmeterreading_in_1) )
    print (" 1. 8. 2 - Meterstand Elektriciteit levering (T2/Laagtarief): %0.3f %s" % (p1_meterreading_in_2,p1_unitmeterreading_in_2) )
    print (" 2. 8. 1 - Meterstand Elektriciteit teruglevering (T1/Normaaltarief): %0.3f %s" % (p1_meterreading_out_1,p1_unitmeterreading_out_1) )
    print (" 2. 8. 2 - Meterstand Elektriciteit teruglevering (T2/Laagtarief): %0.3f %s" % (p1_meterreading_out_2,p1_unitmeterreading_out_2) )
    ...
    print ("96. 3.10 - Actuele Limiter Threshold Elektriciteit: %s" % p1_current_switch_position )
    print ("31. 4. 0 - Actuele Fuse Supervision Threshold Elektriciteit: %s %s" % (p1_fuse_supervision_threshold, p1_unit_fuse_supervision_threshold))
    ...
    for channel in channellist:
        if channel.id != 0:
            ...
            print ("96. 1. 1 - Meternummer %s: %s" % (channel.type_desc, channel.equipment_id) )
            if p1_dsmr_version in ["50213"]:                
    ...
    print ("CRC: %s" % p1_telegram_CRC_match )
...
#New filename every day
    csv_filename=datetime.strftime(datetime.utcnow(), "P1_"+p1_equipment_id+"_%Y-%m-%d_"+str(log_interval)+"s.csv" )
    try:
...
#    print(query)
    try:
        db = mysql.connector.connect(user=p1_mysql_user, password=p1_mysql_passwd, host="127.0.0.1", database=p1_mysql_db,port=3307)
        c = db.cursor()
        c.execute (query)
        db.commit()
...
# Prepare the data
#    json = '{"time":'+ '"'+ p1_timestamp +'"'
    json = '{'
    json += '"p1MeterreadingIn1":' + '"%0.3f"'%p1_meterreading_in_1 
    json += ',"p1MeterreadingIn2":' + '"%0.3f"'%p1_meterreading_in_2 
    json += ',"p1MeterreadingOut1":' + '"%0.3f"'%p1_meterreading_out_1
    json += ',"p1MeterreadingOut2":' + '"%0.3f"'%p1_meterreading_out_2
    json += ',"p1CurrentPowerIn":' + '"%0.3f"'%p1_current_power_in
    json += ',"p1InstantaneousCurrentL1":' + '"%0.3f"'%p1_instantaneous_current_l1
    json += ',"p1VoltageL1":' + '"%0.3f"'%p1_voltage_l1
    json += ',"p1CurrentPowerOut":' + '"%0.3f"'%p1_current_power_out
    json += ',"p1MeterreadingPrd":' + '"%0.3f"'%p1_meterreading_prd
    json += '}'   
    
    #json_channelA = '{"time":'+ '"'+ p1_channel_1.timestamp +'"'
    json_channelA = '{'
    json_channelA += '"p1Channel1Meterreading":' + '"%0.3f"'%p1_channel_1.meterreading
    json_channelA += '}'   
    
 
    # Prepare the URL 
    url  ="http://" + emoncms_hostname + "/" +  "input/post.json?node=P1_Synology&apikey=" + emoncms_apikey + "&fulljson=" + urllib.parse.quote(json)
    url_channelA  ="http://" + emoncms_hostname + "/" +  "input/post.json?node=P1_Synology&apikey=" + emoncms_apikey + "&fulljson=" + urllib.parse.quote(json_channelA)
    print(url)
    print(url_channelA)
# Send the data to emoncms
    ...
    try:
        response_channelA = urllib.request.urlopen(url_channelA)
    except URLError as response_channelA:
        if hasattr(response_channelA, 'reason'):
            print ("Fout bij benaderen van %s. Response: %s %s" % (emoncms_hostname, response_channelA.status, response_channelA.reason))

        elif hasattr(response, 'code'):
            print ("Fout bij het verwerken van request door %s. Response: %s %s" % (emoncms_hostname, response_channelA.status, response_channelA.code))
    else:
        print ("P1 telegram verwerkt door %s op: %s (UTC) / %s (Local). Response: %s %s" % (emoncms_hostname, p1_timestamp_utc, p1_timestamp,response_channelA.status, response_channelA.reason))
...
#Domoticz output P1  #
######################
def domoticz_p1_telegram():
    print ("P1 telegram versturen naar Domoticz %s / %s / %s." % (domoticz_hostname, domoticz_port, domoticz_p1device))
# Prepare the data
    json = domoticz_p1device
    json += "&nvalue=0&svalue=" + "%0.0f"%(p1_meterreading_in_1*1000)
    json += ";" + "%0.0f"%(p1_meterreading_in_2*1000)
    json += ";" + "%0.0f"%(p1_meterreading_out_1*1000)
    json += ";" + "%0.0f"%(p1_meterreading_out_2*1000)
    json += ";" + "%0.0f"%(p1_current_power_in*1000)
    json += ";" + "%0.0f"%(p1_current_power_out*1000)
    # Prepare the URL 
    url  ="http://" + domoticz_hostname+ ":" + domoticz_port + "/" +  "json.htm?type=command&param=udevice&idx=" + json
    print(url)
# Send the data to domoticz
    try:
        ...
        if hasattr(response, 'reason'):
            print ("Fout bij benaderen van %s. Response: %s %s" % (domoticz_hostname, response.status, response.reason))

        elif hasattr(response, 'code'):
            print ("Fout bij het verwerken van request door %s. Response: %s %s" % (domoticz_hostname, response.status, response.code))
    else:
        print ("P1 telegram verwerkt door %s op: %s (UTC) / %s (Local). Response: %s %s" % (domoticz_hostname, p1_timestamp_utc, p1_timestamp,response.status, response.reason))

    return    

############################
#Domoticz output advanced  #
############################
def domoticz_telegram():
#FromGrid
    print ("FromGrid Telegram versturen naar Domoticz %s / %s / %s." % (domoticz_hostname, domoticz_port, domoticz_fgdevice))
# Prepare the data
    json = domoticz_fgdevice
    json += "&nvalue=0&svalue=" + "%0.3f"%(p1_current_power_in)
    json += ";" + "%0.3f"%(p1_meterreading_in_1+p1_meterreading_in_2)
    # Prepare the URL 
    url  ="http://" + domoticz_hostname+ ":" + domoticz_port + "/" +  "json.htm?type=command&param=udevice&idx=" + json
    print(url)
# Send the data to domoticz
    try:
        ...
        print ("FromGrid Telegram verwerkt door %s op: %s (UTC) / %s (Local). Response: %s %s" % (domoticz_hostname, p1_timestamp_utc, p1_timestamp,response.status, response.reason))

#Togrid
    print ("ToGrid Telegram versturen naar Domoticz %s / %s / %s." % (domoticz_hostname, domoticz_port, domoticz_tgdevice))
# Prepare the data
    json = domoticz_tgdevice
    json += "&nvalue=0&svalue=" + "%0.3f"%(p1_current_power_out)
    json += ";" + "%0.3f"%(p1_meterreading_out_1+p1_meterreading_out_2)
    # Prepare the URL 
    url  ="http://" + domoticz_hostname+ ":" + domoticz_port + "/" +  "json.htm?type=command&param=udevice&idx=" + json
    print(url)
# Send the data to domoticz
    try:
        ...
        print ("ToGrid Telegram verwerkt door %s op: %s (UTC) / %s (Local). Response: %s %s" % (domoticz_hostname, p1_timestamp_utc, p1_timestamp,response.status, response.reason))

#Production
    print ("Production Telegram versturen naar Domoticz %s / %s / %s." % (domoticz_hostname, domoticz_port, domoticz_pdevice))
# Prepare the data
    json = domoticz_pdevice
    json += "&nvalue=0&svalue=" + "%0.3f"%(p1_current_power_prd)
    json += ";" + "%0.3f"%(p1_meterreading_prd)
    # Prepare the URL 
    url  ="http://" + domoticz_hostname+ ":" + domoticz_port + "/" +  "json.htm?type=command&param=udevice&idx=" + json
    print(url)
# Send the data to domoticz
...
#Consumption
    print ("Consumption Telegram versturen naar Domoticz %s / %s / %s." % (domoticz_hostname, domoticz_port, domoticz_cdevice))
# Prepare the data
    json = domoticz_pdevice
    json += "&nvalue=0&svalue=" + "%0.3f"%(p1_current_power_in - p1_current_power_out + p1_current_power_prd)
    json += ";" + "%0.3f"%((p1_meterreading_in_1+p1_meterreading_in_2) - (p1_meterreading_out_1+p1_meterreading_out_2) +(p1_meterreading_prd))
    # Prepare the URL 
    url  ="http://" + domoticz_hostname+ ":" + domoticz_port + "/" +  "json.htm?type=command&param=udevice&idx=" + json
    print(url)
# Send the data to domoticz
    try:
        ...
        print ("Consumption Telegram verwerkt door %s op: %s (UTC) / %s (Local). Response: %s %s" % (domoticz_hostname, p1_timestamp_utc, p1_timestamp,response.status, response.reason))
...
parser.add_argument("-l", "--loginterval", help="Log frequency in seconds, default=1", default=0, type=int)
...
parser.add_argument("-domoticzop1", "--domoticzoutputp1", help="Output Domoticz Virtual P1 ==EXPERIMENTAL==, default='N'", default='N', choices=['Y', 'N'])
parser.add_argument("-domoticzhost", "--domoticzhostname", help="Domoticz hostname, default='localhost'",default='localhost')
parser.add_argument("-domoticzport", "--domoticzport", help="Domoticz port on host, default = '8080'", default='8080')
parser.add_argument("-domoticzp1", "--domoticzp1device", help="Domoticz P1 device idx")

parser.add_argument("-domoticzo", "--domoticzoutput", help="Output Domoticz Advanced ==EXPERIMENTAL==, default='N'", default='N', choices=['Y', 'N'])
#parser.add_argument("-domoticzhost", "--domoticzhostname", help="Domoticz hostname, default='localhost'",default='localhost')
#parser.add_argument("-domoticzport", "--domoticzport", help="Domoticz port on host, default = '8080'", default='8080')
parser.add_argument("-domoticzfg", "--domoticzfgdevice", help="Domoticz FromGrid device idx")
parser.add_argument("-domoticztg", "--domoticztgdevice", help="Domoticz ToGrid device idx")
parser.add_argument("-domoticzp", "--domoticzpdevice", help="Domoticz Production device idx")
parser.add_argument("-domoticzc", "--domoticzcdevice", help="Domoticz Consumption device idx")
...
parser.add_argument("-d", "--database", help="MySQL database name, default=p1'", default='P1_log')
parser.add_argument("-v", "--version", help="DSMR COM-port setting version, default=5'", choices=['2','3','4','5'], default='5')
...
domoticz_output_p1 = ( args.domoticzoutputp1 == "Y")
domoticz_output_adv = ( args.domoticzoutput == "Y")
log_interval = args.loginterval
dsmr_version = args.version

if domoticz_output_p1 and (args.domoticzp1device == None):
    parser.print_help();
    print ("\r")
    print("%s: error: If -domoticzop1/--domoticzoutputp1 is 'Y', the following arguments are required: -domoticzp1/--domoticzp1device" % progname)
    print ("Program aborted.")
    sys.exit()

if domoticz_output_adv and (args.domoticzfgdevice == None or args.domoticztgdevice == None or args.domoticzcdevice == None or args.domoticzpdevice == None):
    parser.print_help();
    print ("\r")
    print("%s: error: If -domoticzo/--domoticzoutput is 'Y', the following arguments are required: -domoticzfg/--domoticzfgdevice , -domoticztg/--domoticztgdevice , -domoticzp/--domoticzpdevice , -domoticzc/--domoticzcdevice." % progname)
    print ("Program aborted.")
    sys.exit()
...
domoticz_hostname = args.domoticzhostname
domoticz_port= args.domoticzport
domoticz_p1device = args.domoticzp1device
domoticz_fgdevice = args.domoticzfgdevice
domoticz_tgdevice = args.domoticztgdevice
domoticz_pdevice = args.domoticzpdevice
domoticz_cdevice = args.domoticzcdevice
...
print ("Domoticz P1 logging      : %s" % domoticz_output_p1)
if domoticz_output_p1:
    print ("- Domoticz hostname     : %s" % domoticz_hostname)
    print ("- Domoticz port         : %s" % domoticz_port)
    print ("- Domoticz P1 device idx: %s" % domoticz_p1device)
print ("Domoticz Advanced logging: %s" % domoticz_output_adv)
if domoticz_output_adv:
    print ("- Domoticz hostname              : %s" % domoticz_hostname)
    print ("- Domoticz port                  : %s" % domoticz_port)
    print ("- Domoticz FromGrid device idx   : %s" % domoticz_fgdevice)
    print ("- Domoticz ToGrid device idx     : %s" % domoticz_tgdevice)
    print ("- Domoticz Production device idx : %s" % domoticz_pdevice)
    print ("- Domoticz Consumption device idx: %s" % domoticz_cdevice)
...
if comport != "TEST":
    ...
    if dsmr_version == '2' or dsmr_version == '3':
        ...
        ser.xonxoff=1
    if dsmr_version == '4' or dsmr_version == '5':
        ser.baudrate = 115200
...
p1_unit_current_threshold=""
p1_fuse_supervision_threshold=0
p1_unit_fuse_supervision_threshold=""
...
#Only proceed if P1 telegram start is recognized.        
    if p1_telegram:
    #build raw telegram for CRC calculation
        if re.match(b'.*(?=/)', p1_raw):
                startFound = True
                endFound = False
                p1_telegram_raw = p1_raw.lstrip()
        elif re.match(b'(?=!)', p1_raw):
                endFound = True
                p1_telegram_raw = p1_telegram_raw + p1_raw
        else:
                p1_telegram_raw = p1_telegram_raw + p1_raw
...
#Check if meter clock is running
                    ...
                    print("Elektra DST indicator: ", p1_line[22:23])
...
#DST, subtract 2 hours to get UTC!  
                    ...
                    p1_timestamp_utc=p1_timestamp_dt.strftime("%Y-%m-%dT%H:%M:%S+02:00")
                    print("Elektra UTC timestamp: ", p1_timestamp_utc)
                else:
                    use_systemtime = True
        ...
        elif p1_line[4:10] == "96.1.4":
#DSMR Version (DSMR V5)
#eg. -0:96.1.4(50213)
            p1_lastpos=len(p1_line)-1
            p1_dsmr_version=p1_line[11:p1_lastpos]
...
#eg. 0-0:96.1.1(204B413655303031353131323039393130)
            #Equipment identifier submeters
#eg. 0-1:96.1.1(3238303039303031303434303132303130)
...
#self, id=None, type_id=None, type_desc=None, equipment_id=None, timestamp=None, meterreading=None, unit=None, valveposition=None
            try:
             if p1_channel==0:
                ...
                p1_equipment_id = bytes.fromhex(p1_equipment_id).decode('ascii')
             if p1_channel==1:
                p1_channel_1.equipment_id=bytes.fromhex(p1_value).decode('ascii')
             elif p1_channel==2:
                p1_channel_2.equipment_id=bytes.fromhex(p1_value).decode('ascii')
             elif p1_channel==3:
                p1_channel_3.equipment_id=bytes.fromhex(p1_value).decode('ascii')
             elif p1_channel==4:
                p1_channel_4.equipment_id=bytes.fromhex(p1_value).decode('ascii')
            except ValueError:
               print("Error FromHex")
               p1_equipment_id=""
...
#eg. 0-0:17.0.0(0999.00*kW)
        ...
        elif p1_line[4:10] == "31.4.0":
#Fuse supervision threshold Electricity
#eg. 1-0:31.4.0(999*A)

            p1_lastpos=len(p1_line)-1
            p1_num_start = p1_line.find("(") +1
            p1_num_end = p1_line.find("*")
            p1_fuse_supervision_threshold=float(p1_line[p1_num_start:p1_num_end])        
            p1_unit_fuse_supervision_threshold=p1_line[p1_num_end+1:p1_lastpos]                 
...
#eg. 0-0:96.3.10(1), default to 1
            ...
            if int(p1_value)==0:
                p1_value="Disconnected"
            elif int(p1_value)==1:
                p1_value="Connected"
            elif int(p1_value)==2:
                p1_value="ready_for_reconnection"
            p1_current_switch_position=p1_value
...
#eg. 1-0:31.7.0.255(001*A)
            ...
            p1_num_start = p1_line.find("(") +1
            p1_num_end = p1_line.find("*")
            p1_instantaneous_current_l1=(float(p1_line[p1_num_start:p1_num_end]))
...
# #eg. 1-0:51.7.0.255(002*A)
            ...
            # p1_num_start = p1_line.find("(") +1
            # p1_num_end = p1_line.find("*")
            # p1_instantaneous_current_l2=(float(p1_line[p1_num_start:p1_num_end]))
...
# #eg. 1-0:71.7.0.255(003*A)
            ...
            # p1_num_start = p1_line.find("(") +1
            # p1_num_end = p1_line.find("*")
            # p1_instantaneous_current_l3=(float(p1_line[p1_num_start:p1_num_end]))
...
#1-0:32.7.0(00234*V) 
            ...
            p1_num_start = p1_line.find("(") +1
            p1_num_end = p1_line.find("*")
            p1_voltage_l1=(float(p1_line[p1_num_start:p1_num_end]))
...
# #1-0:52.7.0(00234*V) 
            ...
            # p1_num_start = p1_line.find("(") +1
            # p1_num_end = p1_line.find("*")
            # p1_voltage_l2=(float(p1_line[p1_num_start:p1_num_end]))
...
# #1-0:72.7.0(00234*V) 
            ...
            # p1_num_start = p1_line.find("(") +1
            # p1_num_end = p1_line.find("*")
            # p1_voltage_l3=(float(p1_line[p1_num_start:p1_num_end]))
...
#self, id=None, type_id=None, type_desc=None, equipment_id=None, timestamp=None, meterreading=None, unit=None, valveposition=None
        ...
        elif p1_line[4:10] == "24.2.3":
#Last 5-minute value delivered to client 'not' temperature corrected (DSMR 5+)
#eg. 0-1:24.2.3(101209110000W)(12785.123*m3)
#eg. 0-1:24.2.3(170102133508W)(00094.909*m3)
            p1_channel=int(p1_line[2:3])
            if p1_lastpos > 15:
...
#DST, subtract 2 hours to get UTC!  
                ...
                p1_channel_timestamp_dt=datetime.strptime("2019-01-01 00:00:00", "%Y-%m-%d %H:%M:%S") 
            
            p1_channel_timestamp=p1_channel_timestamp_dt.strftime("%Y-%m-%d %H:%M:%S")
            #print("Channel UTC timestamp: ", p1_channel_timestamp)
            p1_lastpos=len(p1_line)-1
            p1_line=p1_line[25:p1_lastpos]
            p1_lastpos=len(p1_line)
            p1_num_start = p1_line.find("(") +1
            p1_num_end = p1_line.find("*")
            #print("p1_value:",p1_line)
            try:
...
#
             ...
             for match in pattern.finditer(p1_telegram_raw):
                packet = p1_telegram_raw[:match.end() + 1]
                checksum = p1_telegram_raw[match.end() + 1:]
             ...
             if checksum.strip():
                ...
                if given_checksum != calculated_checksum:
                    ...
                    p1_telegram_CRC_match = True


#Output if we have the start-of-telegram AND the 'log_interval' between two outputs have gone by
             if comport == "TEST":
                if dsmr_version == "5":
                   #sleep(1)
                   print("sleeping")
                    ...
                    sleep(10)                
             output_delta = datetime.now() - last_output
             output_delta_seconds = output_delta.total_seconds()
             print("output_delta_seconds: ", output_delta_seconds)
             if p1_telegram == True and output_delta_seconds >= log_interval and p1_telegram_CRC_match:
                last_output = datetime.now()
                print("Valid Data to be processed")
################################################################
...
#Output to Domoticz
                if domoticz_output_p1: domoticz_p1_telegram()
                if domoticz_output_adv: domoticz_telegram()                  
################################################################
                p1_telegram=False
                p1_telegram_raw = b''
                #sleep(log_interval)  ##just SLEEP is not the right solution as the input buffer gets filled anyway, I we have to keep interpreting until the set number of seconds have passed.
